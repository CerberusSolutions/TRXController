...
- RadioReference UK (online, optional; radioreferenceuk.co.uk): `src/main/identities/rruk.ts` is
  ...
  flight 1.2 s apart, 10-minute backoff per frequency, and a **halt** on any refusal from the server (`haltFor`, `RrukStatus.halted`): a rejected key (401 / 403, or a message about the key) stops every lookup, forced ones included, until the key is changed or a Test passes; a rate limit, a locked IP address or too many locations (429, or the message says so) and any other refusal pause lookups for `RRUK_HALT_MS` (15 min); an unreachable server pauses them for a minute. The RRUK operators asked for this on 25 Sep 2026 after clients kept sending on a 401. The Data dialog shows the server's own message with what to do (a rejected key: enter the right one and Test; a lock or limit: contact RRUK support), and the hero's lookup line carries it on every frequency while halted. **Lookups run only with a key that has passed a Test** (`RrukSettings.tested`, the operators' ask on 25 Sep 2026): saving a key clears it, a passed Test sets it (`setTested`), a refusal about the key clears it again, so a wrong key is sent at most once; `RrukService.info` says `RRUK_UNTESTED` on every frequency meanwhile and the Data dialog shows an amber "Key not tested yet" until the green tick. **The API key is the
