...
/** The pair as stored on a candidate: the point when believable, else nulls. */
const pointOrNull = (lat: number | null | undefined, lon: number | null | undefined): { lat: number | null; lon: number | null } => point(lat, lon) ?? { lat: null, lon: null };
...
export function normaliseCandidates(v: unknown): Candidate[] {
  ...
  for (const item of v) {
    ...
    const c: Candidate = {
      ...
      ...pointOrNull(num(o['lat']), num(o['lon'])),
    };
