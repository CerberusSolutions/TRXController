...
export function rowMatches(r: ReceptionRow, filter: string): boolean {
  ...
  if (!f) return true;
  const hay = [r.name, r.system, r.scanlist, r.objectType, r.mode, r.site, r.squelch, r.tone, (r.frequencyHz / 1e6).toFixed(6), r.tgid ?? '', r.radioId ?? '', r.radioCallsign ?? '', r.radioName ?? '', r.source ?? '', r.scannerName ?? '', r.wtr ?? '', r.rrName ?? '', r.rrSystem ?? '', r.rpt ?? '', ...(r.candidates ?? []).map((c) => c.name)]
    .join(' ')
    .toLowerCase();
